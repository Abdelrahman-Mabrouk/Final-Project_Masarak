...
class _TripDetailsState extends State<TripDetails> {
  ...
  Future<void> readJson() async {
    final String stations = await rootBundle.loadString('assets/jsonFiles/metroLinesData.json');
    final List<dynamic> jsonData = json.decode(stations);

    // Cast each item to a Map and store the full data.
    data = jsonData.map((item) => item as Map<dynamic, dynamic>).toList();

    // Extract only the values of the "name" key into a list of strings.
    names = data
        .map((item) => item['name']) // Extract the "name"
        .whereType<String>()
        .cast<String>() // Filter out any nulls or non-string values
  ...
  Widget build(BuildContext context) {
    return noDirection
      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
            DirectionTop(fromStation: names[0], toStation: names[11]),
            Container(
